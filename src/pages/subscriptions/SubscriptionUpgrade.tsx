...
const SubscriptionUpgrade: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-white via-blue-50 to-blue-100 relative">
      ...
      <div className='fixed top-4 left-4 bg-slate-200 rounded-md hover:bg-slate-300 p-4 active:scale-95' onClick={() => navigate("/dashboard")}>
        <ChevronLeft />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-full">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-black mb-2 mt-2">Choose Your Plan</h1>
          <p className="text-gray-700">Upgrade to unlock unlimited posts and premium features</p>
        </div>
        ...
        {/* Current Plan Status */}
        {subscriptionStatus && (
          <div className="mb-8 backdrop-blur-lg bg-white rounded-2xl p-6 shadow-lg">
            <div className="text-center">
              <h2 className="text-xl font-semibold text-black mb-4">Current Plan</h2>
              <div className="inline-flex items-center px-4 py-2 bg-slate-300/10 rounded-lg mb-4">
                <span className="text-lg font-semibold text-black capitalize">{subscriptionStatus.plan}</span>
              ...
              {subscriptionStatus.plan !== "free" && (
                <button
                  ...
                  className="px-6 py-3 bg-linear-to-r from-red-800  to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg transition-all duration-200 flex items-center text-sm font-medium mx-auto"
                >
                  <Cog />
                  <span className='ml-2'>{openingPortal ? 'Opening...' : 'Manage Subscription'}</span>
                </button>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 max-w-4xl mx-auto">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative backdrop-blur-lg border rounded-2xl p-6 transition-all duration-200 ${plan.popular
                ? 'bg-linear-to-br from-purple-500/90 to-blue-500/90 border-purple-500/70'
                : 'bg-white/10 border-white/20 hover:bg-white/15'
                } ${plan.current ? 'ring-2 ring-green-500/50' : ''} flex flex-col justify-between`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  <span className="bg-linear-to-r from-purple-600 to-blue-600 text-white px-4 py-1 rounded-full text-sm font-medium">
                    Most Popular
                  </span>
                </div>
              )}
              ...
              <div className="text-center mb-6">
                <h3 className={`text-xl font-semibold mb-2 ${plan.popular ? "text-white" : "text-black"}`}>{plan.name}</h3>
                <div className="mb-2">
                  <span className="text-3xl font-bold text-emerald-500">{plan.price}</span>
                  <span className={`${plan.popular ? "text-white" : "text-black"}`}>/{plan.period}</span>
                </div>
                <p className={`${plan.popular ? "text-white" : "text-black"} text-sm`}>{plan.description}</p>
              </div>

              <ul className="space-y-3 mb-6">
                {plan.features.map((feature, index) => (
                  <li key={index} className={`flex items-center text-sm ${plan.popular ? "text-white" : "text-black"}`}>
                    <svg className="w-4 h-4 text-green-400 mr-3 shrink-0" fill="currentColor" viewBox="0 0 20 20">
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-lg font-medium transition-all duration-200 ${plan.current
        ...
        {/* Plan Comparison */}
        <div className="mb-8 backdrop-blur-lg bg-white border border-white/20 rounded-2xl p-6 shadow-lg">
          <h2 className="text-xl font-semibold text-black mb-6 text-center">Plan Comparison</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-white border-2 border-gray-200 rounded-lg">
                  <span className="text-gray-700">Posts per month</span>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-linear-to-r from-purple-500 to-blue-500 rounded-lg">
                  <span className="text-white">Posts per month</span>
                  <span className="text-white font-medium">Unlimited</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-linear-to-r from-purple-500 to-blue-500 rounded-lg">
                  <span className="text-white">Support level</span>
        ...
        {/* FAQ Section */}
        <div className="backdrop-blur-lg bg-white border border-white/20 rounded-2xl p-6 shadow-lg">
          <h2 className="text-xl font-semibold text-black mb-4">Frequently Asked Questions</h2>
          <div className="space-y-4">
            <div>
              <h3 className="text-sm font-medium text-black mb-1">Can I upgrade anytime?</h3>
              <p className="text-sm text-gray-700">Yes, you can upgrade from Free to Premium at any time. Changes take effect immediately.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-700">Yes, you can cancel your Premium subscription anytime and return to the Free plan.</p>
            </div>
          </div>
        </div>
      </div>
